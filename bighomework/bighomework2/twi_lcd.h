...
#define LCD_SLAVE_ADDRESS 0x27 //从机地址PCF8574(A2-0:111)// LCD进入模式设置位(LCD_ENTRYMODESET=0x04)
#define LCD_ENTRYSHIFT 0x01 //S位=1，显示移位，=0不移位
#define LCD_ENTRYINC 0x02 //I/D位=1，显示左移(递增)
...
unsigned char TWI_Write_LCD(unsigned char uc_data)
{
	...
	if(TWI_Get_State_Info()!=TW_MT_DATA_ACK)
	return 0; //不成功
	TWI_Stop();
	return 1; //成功
}
...
{
	unsigned char high4bit = uc_data & 0xf0; //取高4位
	unsigned char low4bit = (uc_data<<4)&0xf0; //取低4位
...
void LCD_Write_Grade(unsigned int uc_tmp)
{
	LCD_Write_String(0,0,"grade:");
	LCD_Write_Char(0, 7, uc_tmp / 1000 + 0x30);
	LCD_Write_Char(0, 8, (uc_tmp / 100) % 10 + 0x30);
	LCD_Write_Char(0, 9, (uc_tmp / 10) % 10 + 0x30);
	LCD_Write_Char(0, 10, uc_tmp % 10 + 0x30);
}
